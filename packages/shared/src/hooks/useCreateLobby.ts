...
import type { CreateLobbyInput, CreateLobbyResult, Json } from "@repo/types";
  ...
  return useMutation<CreateLobbyResult, Error, CreateLobbyInput>({
    mutationFn: async (input) => {
      const { data, error } = await supabase.rpc("rpc_create_lobby", {
        p_title: input.title,
        // input.questions structurally matches Json (string/string[] fields), but plain
        // interfaces lack Json's index signature — cast through unknown like the RPC's own
        // response below.
      ...
      // lobby_to_json/question_to_json (supabase/migrations) guarantee this shape at runtime.
