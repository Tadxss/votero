...
Votero — a QR-code-based group voting/polling app, shipping to iOS, Android, and Web from a single Turborepo monorepo with shared business logic. See [TECH_STACK.md](TECH_STACK.md) for the mandated stack and [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) for the concrete product design (Postgres schema, RLS policy design, Edge Functions, Realtime channel design, build order, and what's deliberately deferred). [docs/CASE_STUDY.md](docs/CASE_STUDY.md) is a portfolio-facing writeup of the same engineering decisions — update it if a decision documented there materially changes. [docs/API_PLAN.md](docs/API_PLAN.md) is a not-yet-built implementation plan for a public API (API keys, creator-side endpoints) — a future session can pick it up directly.

**Status**: backend (schema, RLS, RPCs, all Edge Functions) and the web app's core flows are built and verified end-to-end, both locally and on the real hosted project deployed live on Vercel — creator/voter UI, optional accounts + lobby history, multi-question surveys (choice and free-text questions, mixable, choice questions optionally multi-select or ranked-choice/instant-runoff), Present Mode, a creator Dashboard, dark mode, CSV/image export, `/terms` + `/privacy`, Vercel Analytics, Sentry error monitoring, a GitHub Actions CI pipeline (lint/type-check/build/e2e on every push), rate limiting + content moderation, an accessibility pass (modal focus trapping, live regions, axe-scanned + jsx-a11y-linted), a load/concurrency test for many-simultaneous-voters, per-lobby branding (logo + accent color) on the vote page and Present Mode, a branded PDF results report, a QR poster generator (A4 flyer/table tent/slide presets), one-question-at-a-time steppers on Present Mode and the manage page, and drag-and-drop editing of a draft lobby's questions/options/order (both at creation and afterward, before voting opens). `apps/mobile` is still just the bare Expo scaffold.
