...
18. **Sharing the QR/link/code, a "danger zone" delete button, and join-by-code**: ✅ *done*. The manage page's QR card gained three actions — **Copy link**, **Copy code** (`navigator.clipboard.writeText`, 2s "Copied! ✓" confirmation), and **Share** (`navigator.share` where supported, e.g. mobile — opens the native share sheet; falls back to copying the link on browsers without it, e.g. desktop) — answering "how does the creator actually get this to people" beyond just the QR image itself. "Delete lobby" moved from a bare text link to a proper `Button variant="danger"` with a trash icon, separated from the primary open/close actions by a divider, so it reads as a distinct, deliberate destructive action rather than a stray link. The home page gained a collapsed-by-default "Have a code? Join a lobby" toggle revealing an inline 8-character code input + Join button (`router.push('/vote/' + code)`) — a fallback for anyone who can't scan (code read aloud, shared as text, no camera access), and the same entry point the mobile app will offer once built. `apps/web/app/page.tsx` switched from a fixed `h-[calc(100vh-4rem)]` to `min-h-[...]` so this optional extra content can never be clipped on a very short viewport — the default (collapsed) state still fits with no scroll, matching the original no-scroll fix. Shared `TrashIcon` extracted to `apps/web/app/_components/icons.tsx` (now used by both `/lobbies` and the manage page). Verified end-to-end via Playwright: copy actions land the right text on the clipboard, Share's fallback path runs without throwing, joining via a freshly created lobby's code lands on the correct `/vote/[code]`, and the home page has no scroll in either the collapsed or expanded state.
